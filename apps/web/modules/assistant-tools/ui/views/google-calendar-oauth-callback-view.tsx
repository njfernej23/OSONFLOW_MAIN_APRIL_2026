...
export const GoogleCalendarOAuthCallbackView = () => {
  const router = useRouter()
  const searchParams = useSearchParams()
  const connectWithOAuthCode = useAction(
    api.private.googleCalendar.connectWithOAuthCode
  )
  ...
  useEffect(() => {
    ...
    const completeOAuth = async () => {
      ...
      const exchangeKey = `google-calendar-oauth:${code}:${state}`
      ...
      try {
        ...
        toast.success(
          result.email
            ? `Connected Google Calendar (${result.email})`
  ...
  return (
    <div className="flex min-h-[50vh] flex-col items-center justify-center gap-3 text-center">
      ...
      <div>
        <p className="text-sm font-medium">Connecting Google Calendar</p>

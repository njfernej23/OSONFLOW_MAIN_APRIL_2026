...
export const usePersistedVoiceConversation = (
  ...
  const finishConversationMutation = useMutation(api.public.aiConversations.finish);

  const conversationIdRef = useRef<Id<"aiVoiceConversations"> | null>(null);
  const conversationPromiseRef = useRef<
    Promise<Id<"aiVoiceConversations"> | null> | null
  >(null);
  ...
  const finishConversation = async () => {
    const activeConversationId =
      ...
      (conversationPromiseRef.current

...
const createProviderRequest = ({
    ...
    if (provider === "telegram") {
        ...
        return {
            ...
            body: JSON.stringify({
                ...
                text: createChannelMessage(payload),
    ...
    if (provider === "whatsapp") {
        if (
            ...
            throw new ConvexError({
                code: "BAD_REQUEST",
                message: "WhatsApp destination is missing access token, phone number ID, or recipient",
            });
        }
...
export const deliverEventAttempt: any = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        const request = createProviderRequest({
            ...
            signature: await createSigningSecret(webhook.signingSecret, canonicalBody),
        });
        ...
        await ctx.runMutation((internal as any).system.integrationWebhooks.recordDeliveryResult, {
            webhookId: args.webhookId,
            organizationId: args.organizationId,
            eventId: args.eventId,
            eventType: args.eventType,

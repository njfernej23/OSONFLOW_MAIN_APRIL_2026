...
export type GoogleCalendarSecretPayload = {
  accessToken?: string
  refreshToken?: string
  expiresAt?: number
  email?: string
...
export const getGoogleCalendarOAuthConfig = (): GoogleOAuthConfig => {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    throw new ConvexError(
      "Google Calendar OAuth is not configured. Set GOOGLE_CALENDAR_CLIENT_ID, GOOGLE_CALENDAR_CLIENT_SECRET, and GOOGLE_CALENDAR_OAUTH_REDIRECT_URI in Convex."
    )
  }
...
export const buildGoogleCalendarAuthorizationUrl = ({
  clientId,
  redirectUri,
  state,
  loginHint,
...
export const serializeGoogleCalendarSecret = (
  payload: GoogleCalendarSecretPayload
) => JSON.stringify(payload)

export const isGoogleCalendarOAuthConfigured = (
  payload: GoogleCalendarSecretPayload | null
